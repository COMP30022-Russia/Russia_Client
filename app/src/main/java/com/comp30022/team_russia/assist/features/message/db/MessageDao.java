...
public interface MessageDao {
    ...
    @Query("SELECT * from message_table WHERE associationId = :associationId ORDER BY id ASC")
    LiveData<List<Message>> getAllMessages(int associationId);

    @Query("SELECT * from message_table WHERE associationId = :associationId ORDER BY id DESC LIMIT 1")
    LiveData<Message> getLastMessage(int associationId);

    @Query("SELECT id from message_table WHERE associationId = :associationId ORDER BY id DESC LIMIT 1")
